...
flowchart LR
    ...
    Copy --> Lines[720 line_bufs]
    Lines --> SGT[720-entry frame_sgt]
...
| `line_bufs[720]` | `struct fpga_drm_device` | Allocated once by `fpga_drm_alloc_frame_buffers()` and reused after the in-flight DMA completes. |
| `frame_sgt` | `struct fpga_drm_device` | Persistent 720-entry SG table, one entry per line buffer. |
...
|---|---|---|
| Frame accepted | Format is XRGB8888, width is 1280, height is 720, pitch is at least 5120 bytes. | `fpga_drm_copy_frame()` |
| Host staging complete | All 720 source lines have been copied into `line_bufs[]`. | `fpga_drm_copy_frame()` |
...
| Frame complete | `xdma_xfer_completion()` returns exactly `FPGA_DRM_FRAME_BYTES`. | `fpga_drm_dma_complete_work()` |
...
copies and uploads the complete 1280x720 frame.
