			...
			bool dma_mapped, int timeout_ms)
{
	...
	while (req->offset < req->total_len) {
		...
		default:
			...
			rv = engine_service(engine, xfer->desc_cmpl_th);
			if (rv < 0) {
				pr_err("Failed to service engine after timeout\n");
			} else if (xfer->state == TRANSFER_STATE_COMPLETED) {
				spin_unlock_irqrestore(&engine->lock, flags);
				rv = 0;
				done += xfer->len;
				break;
			} else if (xfer->state == TRANSFER_STATE_FAILED) {
				spin_unlock_irqrestore(&engine->lock, flags);
				rv = -EIO;
				break;
			}
...
static int xdma_count_line_descriptors(struct sg_table *sgt,
				       unsigned int line_size,
				       unsigned int line_count,
				       unsigned int *desc_count)
{
	struct scatterlist *sg;
	unsigned int line_remaining = line_size;
	unsigned int lines_done = 0;
	unsigned int count = 0;
	int i;

	if (!line_size || !line_count || !desc_count)
		return -EINVAL;

	for_each_sg(sgt->sgl, sg, sgt->nents, i) {
		unsigned int sg_remaining = sg_dma_len(sg);
		...
		while (sg_remaining) {
			...
			len = min_t(unsigned int, sg_remaining, line_remaining);
			len = min_t(unsigned int, len, desc_blen_max);
			if (!len)
				return -EINVAL;
	...
	if (lines_done != line_count || line_remaining != line_size)
		return -EINVAL;
...
ssize_t xdma_xfer_submit_lines_nowait(void *cb_hndl, void *dev_hndl,
				int channel, bool write, u64 ep_addr,
				struct sg_table *sgt, bool dma_mapped,
				unsigned int line_size, unsigned int line_count)
{
	struct xdma_dev *xdev = (struct xdma_dev *)dev_hndl;
	struct xdma_engine *engine;
	struct xdma_io_cb *cb = (struct xdma_io_cb *)cb_hndl;
	...
	if (!dev_hndl || !cb_hndl || !sgt || !line_size || !line_count)
		return -EINVAL;
	...
	if (!write || !engine->streaming) {
		pr_info("%s line-packetized nowait only supports H2C streaming.\n",
			engine->name);
		return -EINVAL;
	}
	...
	if (!dma_mapped) {
#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 16, 0)
		nents = pci_map_sg(xdev->pdev, sgt->sgl, sgt->orig_nents, dir);
#else
		nents = dma_map_sg(&xdev->pdev->dev, sgt->sgl,
				    sgt->orig_nents, dir);
#endif
		if (!nents) {
			pr_info("map sgl failed, sgt 0x%p.\n", sgt);
	...
	rv = xdma_count_line_descriptors(sgt, line_size, line_count,
					 &desc_count);
	if (rv < 0)
		goto unmap_sgl;

	if (desc_count > engine->desc_max) {
		pr_info("%s needs %u descriptors, max %u.\n", engine->name,
			desc_count, engine->desc_max);
		rv = -EINVAL;
		goto unmap_sgl;
	}
	...
	req->sgt = sgt;
	req->ep_addr = ep_addr;
	req->total_len = line_size * line_count;
	req->sw_desc_cnt = desc_count;
	req->cb = cb;
	cb->req = req;
	...
	spin_lock_irqsave(&engine->lock, flags);
	if (engine->desc_used || engine->running ||
	    !list_empty(&engine->transfer_list)) {
		spin_unlock_irqrestore(&engine->lock, flags);
		rv = -EBUSY;
		goto unlock_desc;
	}

	if (engine->desc_idx + desc_count > engine->desc_max)
		engine->desc_idx = 0;
	...
	xfer->res_bus = engine->cyclic_result_bus +
			(sizeof(struct xdma_result) * engine->desc_idx);
	xfer->desc_index = engine->desc_idx;
	xfer->desc_adjacent = desc_count;
	xfer->desc_num = desc_count;
	xfer->desc_cmpl_th = desc_count;
	xfer->last_in_request = 1;
	xfer->sgt = sgt;
	xfer->cb = cb;
	if (!dma_mapped)
	...
	transfer_desc_init(xfer, desc_count);
	...
	for_each_sg(sgt->sgl, sg, sgt->nents, i) {
		...
		while (sg_remaining) {
			struct xdma_desc *desc = xfer->desc_virt + desc_used;
			unsigned int len = min_t(unsigned int, sg_remaining,
						 line_remaining);
			u32 control = XDMA_DESC_EOP;

			len = min_t(unsigned int, len, desc_blen_max);
			xdma_desc_set(desc, addr, ep_addr, len, dir);
			...
			if (desc_used == desc_count - 1)
				control |= XDMA_DESC_STOPPED | XDMA_DESC_COMPLETED;

			rv = xdma_desc_control_set(desc, control);
			if (rv < 0) {
				spin_unlock_irqrestore(&engine->lock, flags);
				goto unlock_desc;
			}
	...
	engine->desc_idx = (engine->desc_idx + desc_count) % engine->desc_max;
	engine->desc_used += desc_count;
	spin_unlock_irqrestore(&engine->lock, flags);
	...
	if (rv < 0) {
		spin_lock_irqsave(&engine->lock, flags);
		engine->desc_used -= desc_count;
		xdma_desc_done(xfer->desc_virt, desc_count);
		spin_unlock_irqrestore(&engine->lock, flags);
		goto unlock_desc;
	}
