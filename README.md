...
AX7203 uses four PCIe 2.0 lanes as the main path between the Linux host and the FPGA. On the host side, `fpga_drm.ko` exposes a DRM/KMS display device and accepts `XRGB8888` framebuffers. The native Linux transport is XDMA H2C into VDMA S2MM; the stream format is one active display line per AXI4-Stream packet, and each line carries `active_width * 4` bytes. The current July 16 hardware export has a requester-side H2C regression and emits no stream `TVALID`, so native uploads are an active recovery item rather than a currently passing path.
...
The current Vivado export includes `xdma_ila`, which observes the H2C stream and bypass activity. Older exports also contained a separate video-output ILA; do not assume it exists without checking the matching `.ltx`.
